...
#region [2 points]
    ...
    function Get-slope {
        ...
        $dec = ($m -replace '0(?=\.)|[\D]' | measure-object -Character).Characters
        if ($dec -gt 5) {
            $dec = 6
            $m -split '\d{6}' -ne '' -join ''
        }
        ...
        $nu = [Math]::Round($nu/$gcd)
        $de = [Math]::Round($de/$gcd)

        $mf = "$nu/$de"

        Write-Host "`n`r The slope is $mf"
        ...
        if($yint -le 0){
            if($yint -eq 0){
                $yint = ''
                write-host " The equation for the line is ${mf}x"
            }else{
                write-host " The equation for the line is ${mf}x  $yint"
            }
            
        }else{
            write-host " The equation for the line is ${mf}x + $abyint"
        }

        echo "[($x1,$y1),($x2,$y2)]" > .\temp-math-output

        if($yint -le 0){
            echo "${mf}x$yint" >> .\temp-math-output
        }else{
            echo "${mf}x+$abyint" >> .\temp-math-output
        }
